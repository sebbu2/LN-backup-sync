...
class News {
	function __construct() {
		global $wln, $wn, $rr;
		global $wln_list, $wln_order, $wln_books;
		global $wn_list, $wn_order, $wn_books;
		global $rr_list, $rr_order, $rr_books;
	...
	function get_wln_lists() {
		global $wln_order;
		return array_keys($wln_order);
	}
	
	function get_wln_books($list) {
		global $wln_order;
		return $wln_order[$list];
	}
};
...
foreach($wln_order as $id=>$list) {
	...
	foreach($list as $entry) {
		...
		if(array_key_exists($entry, $cor_wln)) {
			$wln2=$cor_wln[$entry];
			if(!is_null($wln2['wn'])&&array_key_exists($wln2['wn'],$wn_books)&&enabled('wn')) {
			...
			if(!is_null($wln2['rr'])&&array_key_exists($wln2['rr'],$rr_books)) {
		...
		if(count($row)>1) {
			...
			++$lines;
			if(count($wln->msg)>0) {
				if(strlen($row['msg']==0)) $row['msg']=implode(', ',$wln->msg);
				else $row['msg'].=', '.implode(', ',$wln->msg);
			}
...
foreach($rr_books as $rr_id=>$entry) {
	...
	if(count($rr2)>0) {
		...
		//if(!in_array($rr_id,array(7015, 53411))) { var_dump($rr_id, $rr2);die(); }
		if(!in_array($rr_id,array(7015,45718,45173,47619,37103,43381,42986,53411))) { var_dump($rr_id, $row['title'], $rr1, $rr2);die(); }
	}
	$name=strtolower(normalize(name_simplify($row['title'], 1)));
	if(array_key_exists($name, $pos)) {
		$pos1=$pos[$name];
